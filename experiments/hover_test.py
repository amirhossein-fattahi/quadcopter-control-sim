...
def main():
    ...
    result = simulate_hover_experiment()

    final_error = np.linalg.norm(result["refs"][-1] - result["states"][-1, 0:3])
    mean_error = np.mean(np.linalg.norm(result["refs"] - result["states"][:, 0:3], axis=1))

    print("Hover test finished.")
    ...
    logs_logger = SimpleLogger(base_dir="results/logs", run_name="hover_test")
    figs_logger = SimpleLogger(base_dir="results/figures", run_name="hover_test")

    logs_logger.save_metadata({
        "experiment": "hover_test",
    ...
    figs_logger.save_figures(rendered["figures"], prefix="hover")
